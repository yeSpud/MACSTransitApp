...
class BusTest {
	...
	fun busesTest() {
		...
		try {
			//Assert.assertArrayEquals(empty, Bus.getBuses(RouteMatch.parseData(Helper.getJSON(Helper.ALL_VEHICLES_EMPTY_JSON)), this.routes))
			//val buses: Array<Bus> = Bus.getBuses(RouteMatch.parseData(Helper.getJSON(Helper.ALL_VEHICLES_JSON)), this.routes)
