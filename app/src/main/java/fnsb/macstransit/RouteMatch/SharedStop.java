...
public class SharedStop extends Stop {
	...
	private SharedStop(String stopID, double latitude, double longitude, Route[] routes) {
		...
		// Copy the rest of the routs into the childRoutes
		this.childRoutes = new Route[routes.length - 1];
		System.arraycopy(routes, 1, this.childRoutes, 0, this.childRoutes.length);

		// Be sure to create the circle parentCircleOptions for this object at this point.
	 ...
	 * @param routes    The parentRoute that this Stop corresponds to.
	 */
	...
	/**
	 * Finds stops that are shared by different childRoutes, as well as current shared stops.
	 *
	 * @param routes             The childRoutes to check for shared stops.
	...
	public static SharedStop[] findSharedStops(Route[] routes, SharedStop[] currentSharedStops) {
		...
		// Check if there is only 1 or less childRoutes being tracked. If there is,
		...
		for (BasicStop basicStop : allStops) {
			...
			// Create an ArrayList of childRoutes to store all the childRoutes that share this basic stop
			ArrayList<Route> sharedRoutes = new ArrayList<>();

			// Find which childRoutes share the current basicStop (by ID). Start by iterating though the childRoutes.
			for (Route r : routes) {
				Log.d("findSharedStops", "Checking parentRoute: " + r.routeName);
				// For each parentRoute, check the stops to see if their stop ID matches the stop ID we are checking.
				for (Stop s : r.stops) {
					// If the stop matches our ID, then add that parentRoute to an array, so that we can add it to a newly created shared stop.
					if (s.stopID.equals(basicStop.stopID)) {
						sharedRoutes.add(r);
						// Since we are only checking the 1 parentRoute, break from this stop for loop to check another stop.
						break;
					}
				}
			}
	...
	/**
	 * Adds the shared stops to the map. TODO Documentation
	 *
	...
	public static void addSharedStop(com.google.android.gms.maps.GoogleMap map, SharedStop[] sharedStops) {
		for (SharedStop sharedStop : sharedStops) {
			...
			for (int index = 0; index < circles.length; index++) {
				Circle circle = sharedStop.addCircle(map, sharedStop.childCircleOptions[index], false);
	...
	/**
	 * Clears all the elements from the shared stop (as in removes the markers and childCircles from the map).
	 *
	...
	public static SharedStop[] clearSharedStops(SharedStop[] sharedStops) {
		...
		for (SharedStop sharedStop : sharedStops) {
			...
			// Get the childCircles from the shared shared stop, and if its not null remove it.
	...
	/**
	 * Gets the childCircles that belong to this Stop.
	 *
	...
	/**
	 * Sets the childCircles corresponding to the Shared Stop.
	 *
	...
	private CircleOptions[] createCircleOptions() {
		...
		for (int index = 0; index < count; index++) {
			...
			// Make the circle parentCircleOptions for the first one the biggest, and the only one that is clickable
			CircleOptions circleOption = new CircleOptions().center(new com.google.android.gms.maps.model.LatLng(this.latitude, this.longitude)).radius(size);
